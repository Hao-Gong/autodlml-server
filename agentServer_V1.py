...
app = Flask(__name__)
websrv_ip = '0.0.0.0'  # default websrv IP for flask
websrv_port = 8124
initFlg=0
agentList=[]
try:
...
def initAgent():
    ...
    infoData={"info":"initAgent","UserName":agentName,"alreadyExist":agentExist,"currentAgentNum":len(agentList),"reset":reset_flg}
...
def postReward():
    ...
    if isinstance(agent,PPOAgent):
        ...
        infoData={"info":"postReward","state":"done"}
    elif isinstance(agent,boAgent):
        result_y, result_x = agent.addSamples(actionInfo=actionInfo,reward=reward)
        infoData={"info":"postReward","state":"done","result_x":result_x,"result_y":result_y}
    elif isinstance(agent,deAgent):
